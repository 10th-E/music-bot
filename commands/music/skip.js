...
if(!message.member.voice.channel) return message.channel.send('Join a VC First').catch(console.error)
...
message.channel.send({ embeds: [skipEmbed], components: [row] }).catch(console.error)
...
collector.on('end', collected => {
  ...
  if(collected.size === Math.ceil(peeps / 2)){
	...
	message.channel.send('Did Not Get Enough Votes To Skip').catch(console.error)
	  return;
  }
})
}
